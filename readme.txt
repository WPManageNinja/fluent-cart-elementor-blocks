...
- Reviews can now be placed inside an Elementor product template. FluentCart attaches them to the post content, which such a template does not render, so a store using one had no way to show reviews at all. Add a review widget wherever you want them; templates you already have are left alone.
- Fixes the review section rendering twice on a page that carries both the Product Content widget and a review widget.
- The review widgets explain themselves in the editor when the Product Reviews module is switched off, or when the product they point at does not show reviews, rather than rendering nothing.
