...
| Suppressing a core auto-render to avoid a duplicate | `ElementorIntegration::preventDuplicateProductReviews()` | Locates core's listener by instance in `$wp_filter` and removes only that one |
